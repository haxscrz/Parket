...
const CarIcon = () => (
  <svg width="40" height="80" viewBox="0 0 40 80" fill="none" xmlns="http://www.w3.org/2000/svg">
    {/* Car body */}
    <path
      d="M12 20 C12 15, 15 10, 20 10 C25 10, 28 15, 28 20 L32 55 C32 60, 30 65, 20 65 C10 65, 8 60, 8 55 L12 20 Z"
      fill="#3b82f6"
      stroke="#1e40af"
      strokeWidth="1.5"
    />
    {/* Windshield */}
    <rect x="13" y="15" width="14" height="12" rx="2" fill="#93c5fd" opacity="0.6" />
    {/* Windows */}
    <rect x="13" y="30" width="14" height="15" rx="1" fill="#93c5fd" opacity="0.4" />
    {/* Headlights */}
    <circle cx="15" cy="68" r="2.5" fill="#fef08a" />
    <circle cx="25" cy="68" r="2.5" fill="#fef08a" />
    {/* Wheels */}
    <circle cx="12" cy="55" r="4" fill="#1f2937" />
    <circle cx="28" cy="55" r="4" fill="#1f2937" />
    <circle cx="12" cy="25" r="4" fill="#1f2937" />
    <circle cx="28" cy="25" r="4" fill="#1f2937" />
  </svg>
);
...
const createParkingLayout = (): { slots: ParkingSlot[], columns: Column[] } => {
  ...
  const EV_ZONE_Y_MAX = startY + 4 * (SLOT_HEIGHT + SLOT_SPACING);
  const PWD_ZONE_Y_MIN = startY + (SLOTS_PER_SECTION - 2) * (SLOT_HEIGHT + SLOT_SPACING);
...
const calculateNavigationStats = (
  ...
  const speed = isNavigating ? Math.max(5, parseFloat((distanceKm / hours).toFixed(1))) : 0;
...
export function ParkingNavigationScreen({ onBack, darkMode }: { onBack: () => void; darkMode?: boolean }) {
  ...
  const [parkingTimeRemaining, setParkingTimeRemaining] = useState(4 * 60 * 60); // 4 hours in seconds
  ...
  // Periodically refresh slot statuses
  useEffect(() => {
            ...
            <DropdownMenuContent align="end" className="w-56">
              {isAutoMode && (
                <>
                  ...
                  <DropdownMenuRadioGroup value={autoStrategy} onValueChange={(v) => setAutoStrategy(v as 'nearest-car' | 'nearest-mall')}>
          ...
          <div className={`border-t ${isDarkMode ? 'border-zinc-800' : 'border-gray-200'}`}>
            <div className="flex gap-2 px-4 py-3 overflow-x-auto">
              <Button
                ...
                onClick={() => {
                  setFilterType('all');
                  setSelectedSlot(null); // Clear selection when changing filter
                }}
        ...
        >
          ...
          <g transform={`translate(${cameraOffset.x}, ${cameraOffset.y}) scale(${zoom})`}>
            ...
            <g transform={`translate(${carPosition.x}, ${carPosition.y}) rotate(${carPosition.angle})`}>
              ...
              <g transform="translate(-20, -40)" filter="url(#carShadow)">
                <CarIcon />
              </g>
            </g>
        ...
        <div className="px-4 pt-4 pb-3">
          <div className="grid grid-cols-4 gap-3">
            <div className="flex flex-col items-center">
              <div className={`p-2 rounded-xl ${isDarkMode ? 'bg-red-500/20' : 'bg-red-50'} mb-1`}>
                <Gauge className="w-4 h-4 text-red-500" />
              </div>
              <span className={`font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{navigationStats.speed}</span>
              <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>km/h</span>
            </div>
            
            <div className="flex flex-col items-center">
              <div className={`p-2 rounded-xl ${isDarkMode ? 'bg-cyan-500/20' : 'bg-cyan-50'} mb-1`}>
                <Navigation2 className="w-4 h-4 text-cyan-500" />
              </div>
              <span className={`font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{navigationStats.distance}</span>
              <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>km</span>
            </div>
            
            <div className="flex flex-col items-center">
              <div className={`p-2 rounded-xl ${isDarkMode ? 'bg-purple-500/20' : 'bg-purple-50'} mb-1`}>
                <Clock className="w-4 h-4 text-purple-500" />
              </div>
              <span className={`font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{navigationStats.eta}</span>
              <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>min</span>
            </div>
            
            <div className="flex flex-col items-center">
              <div className={`p-2 rounded-xl ${isDarkMode ? 'bg-green-500/20' : 'bg-green-50'} mb-1`}>
                <Fuel className="w-4 h-4 text-green-500" />
              </div>
              <span className={`font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{navigationStats.gasSaved}</span>
